...
axs[2].pie(vsum,labels=['Bikes','Parts','Clothing'], colors=colours,autopct='%1.1f%%')
...
parts_month  = tot_parts.groupby(['month'])[['value','quantity']].sum()
...
axs[0].bar(months, pmr, bottom = bmr, label='Parts',color='tab:blue')
axs[0].bar(months, cmr, bottom = bmr+pmr, label='Clothing',color='tab:orange')
axs[0].set_ylabel('Sales Value (£)')
...
axs[1].bar(months, pmu, bottom = bmu, label='Parts',color='tab:blue')
axs[1].bar(months, cmu, bottom = bmu+pmu, label='Clothing',color='tab:orange')
axs[1].set_ylabel('Units Sold')
